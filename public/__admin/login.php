...
if (isset($_POST["submit"])) {
    ...
    $pwd = secure_string($_POST['motDePasse']);
    ...
    ';
        ...
        $sql = 'SELECT * FROM general_information WHERE login = :login';
        ...
        if ($user) {
            if ($pwd == $user['mdp']) {
                $_SESSION['admin-123Toner'] = $user['id'];
                header("Location: ./");
            } else {
                echo '
                <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script>
...
<head>
    ...
    <title>123Toner - Administration | Login</title>
...
<body x-data="main" class="relative overflow-x-hidden font-nunito text-sm font-normal antialiased" :class="[ $store.app.sidebar ? 'toggle-sidebar' : '', $store.app.theme === 'dark' || $store.app.isDarkMode ?  'dark' : '', $store.app.menu, $store.app.layout,$store.app.rtlClass]">
    ...
    <div class="screen_loader animate__animated fixed inset-0 z-[60] grid place-content-center bg-[#fafafa] dark:bg-[#060818]">
        <h1 class="text-2xl font-bold text-white dark:text-white"> Veuillez patienter...</h1>
    </div>
    ...
    <div class="main-container min-h-screen text-black dark:text-white-dark">
        <div x-data="auth">
            ...
            <div class="relative flex min-h-screen items-center justify-center  bg-cover bg-center bg-no-repeat px-6 py-10 dark:bg-[#060818] sm:px-16">
                ...
                <div class="relative  max-w-[870px] rounded-md bg-[linear-gradient(45deg,#fff9f9_0%,rgba(255,255,255,0)_25%,rgba(255,255,255,0)_75%,_#fff9f9_100%)] p-2 dark:bg-[linear-gradient(52.22deg,#0E1726_0%,rgba(14,23,38,0)_18.66%,rgba(14,23,38,0)_51.04%,rgba(14,23,38,0)_80.07%,#0E1726_100%)]">
                    <div class="relative flex flex-col justify-center rounded-md bg-white/60 backdrop-blur-lg dark:bg-black/50 px-6  py-20">

                        <div class="mx-auto w-full max-w-[440px]" style="padding-left: 3rem;padding-right: 3rem">
                            <div class="mb-10">
                                <h1 class="text-3xl font-extrabold uppercase !leading-snug text-primary md:text-4xl text-center">Se connecter </h1>
                            </div>
                            <form class="space-y-5 dark:text-white" action="" method="post">
                                <div>
                                    <label for="Email">Login</label>
                                    <div class="relative text-white-dark">
                                        <input id="Email" type="text" name="login" placeholder="Email ou nom d'utilisateur" class="form-input ps-10 placeholder:text-white-dark" />
                                ...
                                <div>
                                    ...
                                    <div class="relative text-white-dark">
                                        <input id="Password" type="password" placeholder="Mot de passe" name="motDePasse" class="form-input ps-10 placeholder:text-white-dark" />
                                        <span class="absolute start-4 top-1/2 -translate-y-1/2">
                                            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                                ...
                                <button type="submit" name="submit" class="btn btn-gradient !mt-6 w-full border-0 uppercase shadow-[0_10px_20px_-10px_rgba(67,97,238,0.44)]">
                                    Se connecter
                                </button>
                            </form>
